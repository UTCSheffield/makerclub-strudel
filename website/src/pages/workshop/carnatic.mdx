...
In the above the dashes (like '`-`') stand for a gap, so for example `Tha - Tha -` has a count of four. Notice the repetition of 'Ta' increasing from 1 to 3. They then repeat with the `Tha` replaced with `Dhi`, `Thom` and finally `Nam`:
...
The above rhythm is a bit difficult to express in strudel, but below is an attempt.
