...
| name   | description                         | example                                                                             |
| ------ | ----------------------------------- | ----------------------------------------------------------------------------------- |
| loopAt | sync sample to a number of cycles   | <MiniRepl client:visible tune={`s("gong").loopAt(2)`} />                            |
| fit    | fit sample to event duration        | <MiniRepl client:visible tune={`s("gong/2").fit()`} />                              |
| chop   | chop sample into equal pieces       | <MiniRepl client:visible tune={`s("gong/2").fit().chop(8)`} />                      |
| slice  | re-order slices                     | <MiniRepl client:visible tune={`s("gong/2").fit().slice(4, "3 0 ~ 2")`} />          |
| splice | slice and change speed              | <MiniRepl client:visible tune={`s("gong/2").splice(4, "3 0 ~ 2")`} />               |
| striate| interlace samples                   | <MiniRepl client:visible tune={`s("gong:1 gong:2").striate(6).cut(1)`} />           |